...
static const char *fonts[] = {"JetBrainsMono Nerd Font Propo:size=12,:style=bold"};
...
static uint32_t colors[][3] = {
    /*               fg          bg          border    */
    [SchemeNorm] = {RGB_HEX_TO_RGBA(BASE_05), RGB_HEX_TO_RGBA(BASE_00),
                    RGB_HEX_TO_RGBA(BASE_03)},
    [SchemeSel] = {RGB_HEX_TO_RGBA(BASE_05), RGB_HEX_TO_RGBA(BASE_02),
                   RGB_HEX_TO_RGBA(BASE_0D)},
    [SchemeUrg] = {0, 0, RGB_HEX_TO_RGBA(BASE_08)},
};
...
static unsigned truncate_icons_after =
    3; /* will default to 1, that is the min */
...
#define MENU_OPTIONS(CMD)                                                      \
  CMD " -f 'Iosevka Jt0w Bold 12' -l 10 -N " BASE_00 " -n " BASE_05            \
      " -M" BASE_01 " -m " BASE_05 " -S " BASE_02 " -s " BASE_05

static const Key keys[] = {
    /* Note that Shift changes certain key codes: c -> C, 2 -> at, etc. */
    /* modifier                  key                 function        argument */
    {MODKEY, XKB_KEY_d, spawn,
     SHCMD(MENU_OPTIONS("foot sh -c \"$(" MENU_OPTIONS("wmenu-run") ")\""))},
    {MODKEY, XKB_KEY_Return, spawn, SHCMD("ghostty")},
    {MODKEY, XKB_KEY_g, togglegaps, {0}},
    {MODKEY, XKB_KEY_v, spawn,
     SHCMD("cliphist list | " MENU_OPTIONS(
         "wmenu") " | cliphist decode | wl-copy")},
